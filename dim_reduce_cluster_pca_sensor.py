...
fig, ax = plt.subplots()
ax.bar(list(range(1,49)), pca.explained_variance_ratio_, linewidth=2, color = 'blue')
plt.axis('tight')
...
pca_new = PCA(n_components = gridSearch.best_estimator_.named_steps['pca'].n_components)
pca_new.fit(X_f)
X_transformed_f = pca_new.transform(X_f)
...
data_em_pca_sensor = np.concatenate((component_list, array_aic, array_bic, array_homo_1, array_comp_1, array_sil_1, array_avg_log), axis =1)

data_km_pca_sensor = np.concatenate((component_list, array_homo_2, array_sil_2, array_var), axis =1)

reconstruction_error_pca_sensor = np.concatenate((np.arange(1,49).reshape(-1,1), reconstruction_error), axis = 1)

file = './data/data_em_pca_sensor.csv'
with open(file, 'w', newline = '') as output:
	writer = csv.writer(output, delimiter=',')
	writer.writerows(data_em_pca_sensor)

file = './data/data_km_pca_sensor.csv'
with open(file, 'w', newline = '') as output:
	writer = csv.writer(output, delimiter=',')
	writer.writerows(data_km_pca_sensor)

file = './data/reconstruction_error_pca_sensor.csv'
with open(file, 'w', newline = '') as output:
	writer = csv.writer(output, delimiter=',')
	writer.writerows(reconstruction_error_pca_sensor)