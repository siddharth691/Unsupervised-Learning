...
rp = random_projection.GaussianRandomProjection(n_components = 48)
...
data_em_rp_sensor = np.concatenate((component_list, array_aic, array_bic, array_homo_1, array_comp_1, array_sil_1, array_avg_log), axis =1)

data_km_rp_sensor = np.concatenate((component_list, array_homo_2, array_sil_2, array_var), axis =1)

reconstruction_error_rp_sensor = np.concatenate((np.arange(1,49).reshape(-1,1), reconstruction_error), axis = 1)

file = './data/data_em_rp_sensor.csv'
with open(file, 'w', newline = '') as output:
	writer = csv.writer(output, delimiter=',')
	writer.writerows(data_em_rp_sensor)

file = './data/data_km_rp_sensor.csv'
with open(file, 'w', newline = '') as output:
	writer = csv.writer(output, delimiter=',')
	writer.writerows(data_km_rp_sensor)

file = './data/reconstruction_error_rp_sensor.csv'
with open(file, 'w', newline = '') as output:
	writer = csv.writer(output, delimiter=',')
	writer.writerows(reconstruction_error_rp_sensor)